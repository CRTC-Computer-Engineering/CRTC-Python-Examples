...
for row in range(0, height, resolution):
    for column in range(0, width, resolution):
        ...
        print("Current Color: " + output_color + str(column))
        print("Next Color: " + get_RGB(column + resolution, row) + str(column + resolution))
        if (output_color != get_RGB(column + resolution, row)): # Is the color you're about to print the same as the last one?
            print("Not a line")
            if(output_color != "white"):
                print("Saving to file")
                output_file.write("\"" + str(column) + " " + str(row) + " " + output_color + "\", ")
        previous_color = output_color # set the next round of previous_color to the current color
output_file.write("""]
